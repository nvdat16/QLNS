...
| Area | Missing |
| :--- | :--- |
| Everything | `tests/Qlns.IntegrationTests` (WebApplicationFactory plus Testcontainers) to verify the partial unique indexes, the conditional updates and the more complex EF queries (the expiry alert window, counting blocking tasks, the manager's user subquery). The EF Core migration (`InitialCreate`) has been generated and applied to a real PostgreSQL 17; it matches `schema.sql` v1.2 table-for-table, including column defaults and the `employee_code_seq` sequence, and sign-in plus the Identity and Core HR smoke tests (`tests/smoke/`) pass against it. |
