...
> **Status:** this is a **database design and reference DDL**. The backend is code-complete and reads and writes these tables correctly. An EF Core migration (`InitialCreate`, under `src/backend/src/Qlns.DataAccess/Migrations`) has been generated from the entity model and applied to a real PostgreSQL 17 — it produces the same 28 tables as `schema.sql`, including column defaults and the `employee_code_seq` sequence — and `seed_roles.sql`/`seed_dev.sql` load cleanly on top of it. What is still missing is an automated integration test suite (`tests/Qlns.IntegrationTests` does not exist yet); today the runtime has only been exercised manually, by real sign-in and the `tests/smoke/` scripts against the migrated database.
